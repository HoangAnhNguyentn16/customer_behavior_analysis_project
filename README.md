# 🛒 ShopEasy - Customer Shopping Behavior Analysis
> **Portfolio Project:** Phân tích dữ liệu hành vi mua sắm của khách hàng bằng Python, SQL Server và Power BI.

---

## 📌 1. Tổng quan (Overview)

**ShopEasy** là một doanh nghiệp bán lẻ trực tuyến đang muốn tìm hiểu sâu về hành vi tiêu dùng của khách hàng nhằm tối ưu hóa doanh số, nâng cao mức độ hài lòng và tỷ lệ giữ chân khách hàng (Customer Retention). 

Dự án này thực hiện quy trình phân tích dữ liệu toàn diện (End-to-End Data Analysis) giúp giải quyết bài toán: **Làm thế nào để doanh nghiệp tận dụng dữ liệu hành vi mua sắm nhằm xác định xu hướng, tối ưu hóa chiến lược tiếp thị và phát triển sản phẩm?**

Dự án bao gồm 3 bước chính:
...
* **Tên tệp:** `customer_shopping_behavior.csv`
* **Quy mô:** 3,900 hàng và 18 cột thông tin.
* **Các trường dữ liệu chính:**
  * `Customer ID`, `Age`, `Gender`: Thông tin nhân khẩu học.
  * `Item Purchased`, `Category`, `Size`, `Color`, `Season`: Thông tin sản phẩm & mùa vụ.
  * `Purchase Amount (USD)`: Giá trị đơn hàng.
  * `Review Rating`: Điểm đánh giá chất lượng sản phẩm.
  * `Subscription Status`, `Previous Purchases`, `Frequency of Purchases`: Lịch sử và mức độ gắn kết của khách hàng.
  * `Shipping Type`, `Discount Applied`, `Payment Method`: Kênh vận chuyển, ưu đãi và phương thức thanh toán.

---

## 🛠️ 3. Công cụ & Công nghệ (Tools & Technologies)

* **Ngôn ngữ lập trình:** Python (`pandas`, `numpy`, `sqlalchemy`, `urllib`, `pyodbc`).
* **Cơ sở dữ liệu:** Microsoft SQL Server (sử dụng T-SQL, CTE, Window Functions).
* **Trực quan hóa:** Power BI Desktop (Power Query, DAX, Interactive Visuals).
* **Môi trường phát triển:** Jupyter Notebook, VS Code, SSMS.
...
3. **Kỹ thuật tạo đặc trưng (Feature Engineering):**
   ...
   * **Chuẩn hóa tần suất mua hàng:** Quy đổi chuỗi tần suất (`Weekly`, `Fortnightly`, `Monthly`,...) sang số ngày tương ứng.
...
### 🗄️ Bước 2: Truy vấn & Phân tích chuyên sâu (SQL)
Thực thi **11 truy vấn chiến lược** trên SQL Server bao gồm:
...
8. **Phân tích tác động của Mùa vụ (Seasonality):** So sánh tổng số lượng đơn hàng và doanh thu giữa các mùa (`Spring`, `Summer`, `Fall`, `Winter`). Tìm ra danh mục sản phẩm (`Category`) dẫn đầu doanh thu trong từng mùa cụ thể.
9. **Đánh giá Phương thức thanh toán:** Tính tỷ lệ phần trăm (%) tần suất sử dụng của các phương thức thanh toán khác nhau (`Credit Card`, `PayPal`, `Venmo`, `Cash`...). Tìm hiểu phương thức thường dùng cho các đơn hàng giá trị lớn (High-value orders).
10. **Phân tích Hiệu suất theo Khu vực (Location/Geography):** Xác định Top 5 khu vực/tiểu bang (`Location`) mang lại tổng doanh thu cao nhất. Tính Giá trị đơn hàng trung bình (AOV - Average Order Value) của các khu vực này.
11. **Tối ưu hóa hàng tồn kho theo Thuộc tính sản phẩm (Size & Color):** Thống kê sự kết hợp phổ biến nhất giữa Kích cỡ (`Size`) và Màu sắc (`Color`) theo từng giới tính, đặc biệt là trong danh mục `Clothing` và `Footwear`.
...
### 7.1 Khởi tạo & Làm sạch dữ liệu (Python)

```bash
# Clone (tải) dự án về máy
git clone [https://github.com/HoangAnhNguyentn16/customer_behavior_analysis_project.git](https://github.com/HoangAnhNguyentn16/customer_behavior_analysis_project.git)
